...
module.exports = class {
	...
	async import() {
		...
		const url = (() => {
			...
			const url = resource.startsWith(`${specifier}/`)
				? resource.slice(specifier.length + 1)
				: `packages/${resource}`;
		...
		let fetched;
		try {
			fetched = await fetch(url, { headers: { Connection: 'close' } });
		} catch (err) {
		...
		if (errors?.length) {
			const error = new Error(
				`Errors found importing "${this.#resource}".\n` +
		...
		this.#uri = (() => {
			...
			// If the bundle is not contained in the project, the uri resolves into the node_modules folder
			const root = resource.startsWith(`${specifier}/`)
				? resource.slice(specifier.length)
		...
		try {
			...
			const error = new BeeError(
				`Error compiling bundle "${this.#resource}".\n` +
					`Error: ${err.message}.\n` +
