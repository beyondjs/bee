...
module.exports = (host, { inspect }) =>
	new (class extends Map {
		...
		async #initialise() {
			...
			const check = await (() =>
				new Promise(resolve => http.get(this.#host, r => resolve(true)).on('error', e => resolve(false))))();
			if (!check) {
				console.error(
					`Could not connect to the BeyondJS dev server: "${this.#host}".\n` +
		...
		async import(resource, version) {
			...
			const bundle = (() => {
				...
				return pkg === project.specifier || project.libraries.includes(pkg)
					? new Bundle(this, module, version)
					: new External(this, resource);
			})();
